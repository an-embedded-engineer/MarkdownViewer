...
1〜3とmode / pane actionの整合は`splitView.ts`が守る。4〜5はAppがglobal tabsとreducer resultを同時更新するintegration boundaryで守る。内部programming errorはthrowし、欠落IDを黙ってfilterしない。stale async responseだけは既存revision guardでignoreする。

### 6.3 初期値とmigration

- 起動時: single、active primary、両group empty、ratio 0.5。
...
| action | 入力 | 契約 |
| --- | --- | --- |
| `open-tab` | paneId, tabId, optional anchor | groupになければ末尾追加し、選択・pane activate。既存memberなら重複せず選択 |
| `select-tab` | paneId, tabId | memberだけ選択。非memberはthrow |
| `activate-pane` | paneId | split中のsecondary、またはprimaryをactive化。singleでsecondary指定はthrow |
| `enable-split` | なし | groupを変更せずmode split、active primary |
| `disable-split` | なし | groupを変更せずmode single、active primary。secondary pending navigationだけclear |
| `close-pane-tab` | paneId, tabId | source groupから削除。activeならsource順の右、なければ左、なければnullへfallback |
| `move-tab` | sourcePaneId, destinationPaneId, tabId | split限定。source削除、destination末尾へdedupe追加・選択、destination activate |
| `reset-root` | なし | mode / ratio維持、両group / selection / navigation clear、active primary |
| `consume-navigation` | paneId, tabId, anchor | identity一致時だけpending clear。不一致stale eventはsame stateを返す |
| `set-requested-ratio` | ratio | 既存finite 0..1 exclusive validation |
...
5. anchorがあれば同paneのpending navigationへ設定する。
...
- source paneのruntime statusがclosed IDならclearする。他pane statusは、そのpaneが同じIDを表示していれば維持する。
...
4. action前後で各paneのactive tab identityを比較し、selectionが変わったpaneのstale `PanePreviewStatus`だけをclearする。destinationが移動対象を既に選択中ならstatusと既存DOM / iframeを維持する。selectionが変わったdestinationは新しいpreviewを生成する。
...
- disable: ordered IDs / active tabsを変更せず、mode single、active primary。hidden secondaryのpending navigationとruntimeをclearする。
...
- active primary / secondaryのselected global tabだけrevisionを増やす。
...
- `moveTab`でtyped action、runtime clear、destination focusを調停する。
...
- group ordered IDsからdataを解決するhelperはID不在をthrowし、silent filterしない。
- existing exported pure policy styleを維持し、mutable module-global stateや重複reducerを追加しない。

### 9.3 `DocumentPane`

- `PaneState.orderedTabIds`順に自paneの`OpenDocumentTab[]` viewを解決する。
- selected tabも同じgroup viewから解決し、global listから任意IDを直接選ばない。
...
- CSSはsingle時2列、split時3列を使い、既存tab width / horizontal overflow / focus-visible / loading / error表示を維持する。
...
- ADRは追加しない。本判断は現時点でTauri component固有であり、横断判断の起票条件を満たさない。

## 14. 影響範囲

| 対象 | 予定変更 |
| --- | --- |
| `markdown-viewer-tauri/src/splitView.ts` | ordered IDs、group actions、invariant / reference helper、既存split semantics更新 |
| `markdown-viewer-tauri/src/splitView.test.ts` | add / select / close / move / toggle / root / invalid invariant test |
| `markdown-viewer-tauri/src/App.tsx` | open、local close、cache eviction、move、pane-local tabs、focus / runtime integration |
| `markdown-viewer-tauri/src/App.css` | split時move buttonを含むtab item grid / hover / focus |
| `markdown-viewer-tauri/src/paneRuntime.test.ts` | move / retained secondary / same ID両groupのguard回帰 |
...
- `README.md`: Tauri機能比較のpane-local group / move概要。
- `markdown-viewer-tauri/README.md`: open先、local close、move button、split off / on、keyboard利用方法と制約。
- `docs/rules/project_overview.md`: global data + pane-local group概要。
- `docs/architecture/overview.md`: state source of truthとopen / close / move flow。
- `docs/architecture/code_patterns.md`: ordered reference group、unreferenced cache eviction、typed atomic move pattern。
- `docs/architecture/common_pitfalls.md`: global close混入、silent missing-ID filter、split toggle時の暗黙merge、focus / stale runtime。
...
- `docs/rules/development_workflow.md`: pane別open / close / move、same document、split復元、keyboard / focus手動matrix。

## 16. テスト設計

### 16.1 `splitView.test.ts`
...
- duplicate ID、active nonmemberなどinternal invariantの拒否。
- 既存split width bounds / pointer ratio / keyboard testsの回帰。

### 16.2 `paneRuntime.test.ts`
...
6. primaryからsecondary、secondaryからprimaryへpointerでmoveし、source fallback、destination selection、active pane、focus、StatusBar / ErrorBanner、Explorer highlightを確認する。
...
9. split offでprimaryを表示しsecondary groupを保持し、split再有効化でsecondary順 / active tabが復元される。
...
12. single / split separator、image viewer、MenuBar / Settings / Recent Folders、responsive widthの既存回帰を確認する。
...
| move中のasync result混線 | source / destination runtime clear、pane / tab / revision guard、DOM connection check | engine固有raceはbugfix |
| split off中secondary cacheがmemoryを使う | 最大2group、document dataは共有、session-only | 大規模documentで問題ならsuspend / evictionを別設計 |
| inline buttonでtab幅が狭い | 30px control追加、既存horizontal overflow維持 | compact tab / overflow menuはUX follow-up |
