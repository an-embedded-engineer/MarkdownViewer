...
| Phase 3 Implementation and docs review | Reopened after Phase 4-a Round 4; temporary StatusBar diagnostics removed for an isolated no-diagnostics publish check |
| Phase 4-a User verification | Diagnostic build no longer reproduced the issue; no-diagnostics build re-verification pending before reusable debug panel implementation |
