...
- 共有確認用に `sample_docs/plantuml.md` を追加した。
...
- Medium 1.3: Tauri devとbundleのruntime directoryをRust resolverに反映した。
- Medium 1.4: Theme切替時はPlantUML CLIを再実行しないようAvalonia / Tauri双方で制御した。
- Low指摘: `.gitignore`、共有sample、runtime option契約、SVG sanitizer、UTF-8 stdout/stderr処理、代表エラー表示を実装・文書へ反映した。ADR候補化とversion確認commandはfollow-upとして残す。
