...
- TypeScript policy (`tabStrip.ts`): tab item全体のhorizontal reveal delta、mouse drag開始threshold、candidate pane IDのdrop target narrowing、pointer座標とTabStrip shell矩形の境界判定、pointer / keyboard表示stateのOR policyをDOM非依存のpure functionで管理する。
