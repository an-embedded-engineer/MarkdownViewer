...
- `ImageViewerDialog`は描画済みvisualを`cloneNode(true)`し、app shellのsiblingに表示する。app shellは`inert`となる。close後はkeyboard起点ならbutton、pointer起点ならactive Markdown previewへfocusを戻し、復帰先がdetach済みの場合もactive previewへfallbackする。
