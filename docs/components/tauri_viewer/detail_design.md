...
- 各paneのTabStripはlocal viewだけを40px固定高の1行tabとして描画し、上端state indicator、横overflow、item全体のmanual reveal、roving tabindex、ArrowLeft / ArrowRight / Home / Endを提供する。split時はactive tabのpane間move buttonに加え、mouseでactive / non-active tabを反対paneのTabStripへdragできる。いずれも既存`move-tab` transitionへ接続する。
