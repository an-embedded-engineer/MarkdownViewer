...
| `tabStrip.ts` | TabStrip内だけを動かすitem全体 + 隣接50% peek reveal delta、6px drag threshold、pane IDをnarrowingするdrop policy、pointer座標とshell矩形の境界判定、pointer / keyboard表示stateのOR policyをpure functionで管理する | [markdown-viewer-tauri/src/tabStrip.ts](../../../markdown-viewer-tauri/src/tabStrip.ts) |
...
| `open_root` | candidate treeとdirectory設定を準備しRootSnapshotをcommit。RootOpenResult（tree / settings / context / presentation）を返す | [viewer_session.rs](../../../markdown-viewer-tauri/src-tauri/src/viewer_session.rs) |
| `open_document` | Rust command。`DocumentStore` current root内のMarkdownはUTF-8本文、HTMLはroot-relative `previewUrl`を排他的responseで返す | [viewer_session.rs](../../../markdown-viewer-tauri/src-tauri/src/viewer_session.rs) |
...
| `render_plantuml_diagrams` | Rust command。`spawn_blocking` で各 source を `java -jar plantuml.jar -tsvg -pipe` に渡し、SVG / エラー HTML を返す | [viewer_session.rs](../../../markdown-viewer-tauri/src-tauri/src/viewer_session.rs) |
| `load_recent_folders` / `record_recent_folder` / `remove_recent_folder` | Rust command。app config JSON の `recentFolders` を読み込み、成功した root folder を最大 10 件で保存し、明示削除を反映する | [viewer_session.rs](../../../markdown-viewer-tauri/src-tauri/src/viewer_session.rs) |
| `load_startup_state` / `load_context_settings` / `patch_context_settings` | Rust command。Theme、logical window size、PlantUML jar pathを型付き app config JSON で読み書きする | [viewer_session.rs](../../../markdown-viewer-tauri/src-tauri/src/viewer_session.rs) |
| `SettingsRepository` | global defaults / Recentとproject JSONを分離し、固定sidecar file lock下でfield patch・migration・atomic replaceを行う | [project_settings.rs](../../../markdown-viewer-tauri/src-tauri/src/project_settings.rs) |
