...
- TabStripはname 1行と上端indicatorでactive / Loading / Rendering / Errorを表し、accessible nameへ状態suffix、Loading / Renderingへ`aria-busy`を公開する。empty、状態遷移、水平overflowにかかわらず40px固定高とし、split時の左右preview上端を一致させる。horizontal scrollbarはpointer enter / leaveとwindow-level座標監視でpointer滞在classを管理し、keyboard `:focus-visible`と合わせてthumbを見せる。座標監視は領域外ではgeometryを読まず、pointer click由来のfocusだけでは表示を維持せず、6px track寸法は維持する。activateまたはitem内controlへfocusした場合はmove / closeを含むitem外枠全体をTabStrip内へ表示する。
