...
- `markdown-viewer-tauri/`: Tauri v2 + React + TypeScript 版のデスクトップアプリ。
  ...
  - globalなopen tabデータを共有しながら、paneごとの所属・順序・選択を持つtab groupとpane-local runtimeを左右2 pane Split Viewで表示する。local closeは最後のgroup参照だけをglobal dataから破棄し、tabは明示操作で反対paneへ移動できる。
